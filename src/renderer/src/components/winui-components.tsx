...
export function WinCheckBox(
	props: WinProps & WinChangeProps<boolean> & { IsChecked?: boolean; ThreeState?: boolean }
): React.JSX.Element {
	...
	return (
		<label className={cx("win-checkbox", checked ? "checked" : undefined, props.class)}>
			<input
				...
				onChange={(event) => {
					...
					callback<unknown>(props, "onChange")?.(event)
				}}
			/>
			<span className="win-checkbox-box" aria-hidden="true" />
			<span className="win-checkbox-content">{contentOf(props, props.children)}</span>
		</label>
	)
}
...
export function WinRadioButton(
	props: WinProps &
		WinChangeProps<WinValue> & { GroupName?: string; Value?: WinValue; IsChecked?: boolean }
): React.JSX.Element {
	const selected = props.IsChecked ?? props.modelValue === (props.Value ?? props.value)
	return (
		<label className={cx("win-radio-button", selected ? "checked" : undefined, props.class)}>
			<input
				{...(domProps(props) as InputHTMLAttributes<HTMLInputElement>)}
				type="radio"
			...
			/>
			<span className="win-radio-circle" aria-hidden="true" />
			<span>{contentOf(props, props.children)}</span>
		</label>
	)
}
...
export function WinRadioButtons(
	props: WinItemProps &
		WinChangeProps<WinValue> & { SelectedIndex?: number; Orientation?: string }
): React.JSX.Element {
	...
	const [selected, setSelected] = useControllable(
		...
		-1,
		callback<WinValue>(props, "onValueChange", "onUpdate:SelectedIndex")
	)
	return (
		<div
			className={cx(
				"win-radio-buttons",
				props.Orientation === "Horizontal" ? "horizontal" : undefined,
				props.class
			)}
		>
			{props.Header && <WinTextBlock Text={props.Header} />}
...
export function WinToggleSwitch(
	props: WinProps &
		WinChangeProps<boolean> & { IsOn?: boolean; OnContent?: ReactNode; OffContent?: ReactNode }
): React.JSX.Element {
	...
	return (
		<label className={cx("win-toggle-switch", on ? "on" : undefined, props.class)}>
			<input
				{...(domProps(props) as InputHTMLAttributes<HTMLInputElement>)}
				type="checkbox"
				checked={Boolean(on)}
			...
			/>
			<span className="win-toggle-track">
				<span className="win-toggle-thumb" />
			</span>
			<span>
				{on ? (props.OnContent ?? props.Content) : (props.OffContent ?? props.Content)}
			</span>
		</label>
	)
}
...
export function WinToggleButton(
	props: WinProps & WinChangeProps<boolean> & { IsChecked?: boolean }
): React.JSX.Element {
	...
	return (
		<WinButton
			{...props}
			className={cx("win-toggle-button", checked ? "checked" : undefined, props.className)}
...
export function WinToggleSplitButton(props: WinProps & WinChangeProps<boolean>): React.JSX.Element {
	return <WinSplitButton {...props} className={cx("win-toggle-split-button", props.className)} />
}
...
export function WinSlider(
	...
	const [value, setValue] = useControllable(
		...
		Number(props.defaultValue ?? props.Minimum ?? 0),
		callback<number>(
			props,
			"onValueChange",
			"onChangeValue",
...
export function WinNumberBox(
	...
	return (
		<label className={cx("win-number-box", props.class)}>
			{props.Header && <WinTextBlock Text={props.Header} />}
			<input
				type="number"
	...
	const inputType =
		props.InputScope === "Password"
			...
			: props.InputScope === "Number"
				...
				: "text"
	return (
		<label className={cx("win-text-box-host", props.class)}>
			{props.Header && <WinTextBlock Text={props.Header} />}
...
export function WinRichEditBox(
	props: WinProps & WinChangeProps<string> & { Value?: string }
): React.JSX.Element {
	return (
		<WinTextBox
			{...props}
			AcceptsReturn
			TextWrapping="Wrap"
			className={cx("win-rich-edit-box", props.className)}
		/>
	)
}
...
export function WinProgressBar(
	...
	const percent = props.IsIndeterminate
		? undefined
		: Math.max(0, Math.min(100, (((props.Value ?? 0) - min) / Math.max(1, max - min)) * 100))
	return (
		<div
			className={cx(
				"win-progress-bar",
				props.IsIndeterminate ? "indeterminate" : undefined,
				props.class
			)}
			style={{ ...props.style, ...commonStyle(props) }}
			role="progressbar"
			aria-valuemin={min}
			aria-valuemax={max}
			aria-valuenow={percent === undefined ? undefined : props.Value}
		>
			<span style={{ width: percent === undefined ? undefined : percent + "%" }} />
		</div>
	)
}

export function WinProgressRing(props: WinProps & { IsActive?: boolean }): React.JSX.Element {
	return (
		<span
			className={cx(
				"win-progress-ring",
				props.IsActive === false ? "inactive" : undefined,
				props.class
			)}
			role="progressbar"
			aria-label={String(props["aria-label"] ?? "Loading")}
		/>
	)
}
export function WinRating(
	props: WinProps &
		WinChangeProps<number> & { Value?: number; MaxRating?: number; IsReadOnly?: boolean }
): React.JSX.Element {
	const [value, setValue] = useControllable(
		props.Value ?? props.value,
		Number(props.defaultValue ?? 0),
	...
	return (
		<div className={cx("win-rating", props.class)}>
			{Array.from({ length: props.MaxRating ?? 5 }, (_, index) => (
				<button
					key={index}
					type="button"
					disabled={props.IsReadOnly}
					className={index < Number(value) ? "filled" : undefined}
...
export function WinInfoBar(
	props: WinProps & { Severity?: string; IsOpen?: boolean; IsClosable?: boolean }
): React.JSX.Element {
	const [open, setOpen] = useControllable(
		props.IsOpen,
		true,
		callback<boolean>(props, "onValueChange", "onUpdate:IsOpen")
	)
	if (!open) return <></>
	return (
		<div
			className={cx(
				"win-info-bar",
				"severity-" + String(props.Severity ?? "Informational").toLowerCase(),
				props.class
			)}
			role="status"
		>
			<div className="win-info-bar-content">
				{props.Title && <WinTextBlock Text={props.Title} FontWeight={600} />}
...
function getPosition(
	...
	const top = placement.startsWith("Top")
		? rect.top - (popupRect?.height ?? 0) - 6
		: rect.bottom + 6
	const left = placement.endsWith("EdgeAlignedRight")
...
export const WinFlyout = forwardRef<
	WinFlyoutHandle,
	WinProps & { Placement?: string; IsLightDismissEnabled?: boolean; Trigger?: ReactNode }
>(function WinFlyout(props, ref) {
	...
	useEffect(() => {
		...
		const update = () =>
			setPosition(
				getPosition(anchor.current, popup.current, String(props.Placement ?? "Bottom"))
			)
			...
			? createPortal(
					<>
						<div
							...
							onPointerDown={() =>
								props.IsLightDismissEnabled !== false && setOpen(false)
							}
						/>
						<div
							ref={popup}
	...
	return (
		<>
			<span
				...
				onClick={() => (trigger ? setOpen(!open) : undefined)}
			>
				{trigger}
			</span>
			{popupContent}
...
>(function WinContentDialog(props, ref) {
	...
	const button = (
		...
		text ? (
			<WinButton
				disabled={!enabled}
				onClick={() => {
					callback<unknown>(props, eventName)?.(undefined)
					close(result)
				}}
			>
				{text}
	...
	return createPortal(
		<div
			className="win-content-dialog-overlay"
			onPointerDown={(event) => {
				if (event.currentTarget === event.target && props.IsLightDismissEnabled) close()
			}}
		>
			<section className="win-content-dialog" role="dialog" aria-modal="true">
				<div className="win-content-dialog-content">
					{title && <WinTextBlock Text={title} FontSize={20} FontWeight={600} />}
					{dialogChildren}
				</div>
				<div className="win-content-dialog-command-space">
					{button(
						primaryText,
						"Primary",
					...
					{button(closeText, "Close", true, "onCloseButtonClick")}
				</div>
			</section>
		</div>,
		document.body
...
export function WinMenuFlyout(
	props: WinProps & { Items?: WinMenuItem[]; FlyoutItems?: WinMenuItem[] }
): React.JSX.Element {
				...
				{items.map((item, index) =>
					item.Separator ? (
						...
						>
							...
							<span>{item.Text ?? itemLabel(item)}</span>
							{item.IsChecked && <span>✓</span>}
						</button>
					)
				)}
			</div>
...
export function WinToolTip(props: WinProps & { Target?: ReactNode }): React.JSX.Element {
	const [open, setOpen] = useState(Boolean(props.IsOpen))
	return (
		<span
			className="win-tooltip-host"
			onMouseEnter={() => setOpen(true)}
			onMouseLeave={() => setOpen(false)}
		>
			{props.Target ?? props.children}
			{open && (
				<span className="win-tooltip" role="tooltip">
...
export function WinTeachingTip(
	props: WinProps & { ActionButtonContent?: ReactNode; CloseButtonContent?: ReactNode }
): React.JSX.Element {
	return (
		<WinPopup {...props} className={cx("win-teaching-tip", props.className)}>
			<WinTextBlock Text={props.Title} FontWeight={600} />
			{props.Subtitle && <WinTextBlock Text={props.Subtitle} TextWrapping="WrapWholeWords" />}
			{props.children}
			{props.ActionButtonContent && (
				<WinButton
					onClick={() => callback<unknown>(props, "onActionButtonClick")?.(undefined)}
				>
					{props.ActionButtonContent}
				</WinButton>
			)}
			{props.CloseButtonContent && (
				<WinButton
					onClick={() => callback<unknown>(props, "onCloseButtonClick")?.(undefined)}
				>
					{props.CloseButtonContent}
				</WinButton>
			)}
...
export function WinExpander(
	props: WinProps & { IsExpanded?: boolean; Header?: ReactNode }
): React.JSX.Element {
	...
	return (
		<section className={cx("win-expander", expanded ? "expanded" : undefined, props.class)}>
			<button
				type="button"
				className="win-expander-header"
				onClick={() => setExpanded(!expanded)}
			>
				<span className="chevron-animate" />
				{props.Header ?? props.Content}
			</button>
			{expanded && <div className="win-expander-content">{props.children}</div>}
		</section>
	)
}
export function WinScrollViewer(
	props: WinProps & { HorizontalScrollMode?: string; VerticalScrollMode?: string }
): React.JSX.Element {
	return (
		<div
			className={cx("win-scroll-viewer", props.className, props.class)}
			style={{
				...props.style,
				...commonStyle(props),
				overflowX: props.HorizontalScrollMode === "Disabled" ? "hidden" : "auto",
				overflowY: props.VerticalScrollMode === "Disabled" ? "hidden" : "auto"
			}}
		>
			{props.children}
		</div>
...
export function WinScrollBar(props: WinProps): React.JSX.Element {
	return <div className={cx("win-scroll-bar", props.class)}>{props.children}</div>
}

export function WinItemsRepeater(props: WinItemProps): React.JSX.Element {
	return <div className={cx("win-items-repeater", props.class)}>{renderItems(props)}</div>
}
export function WinItemsView(
	props: WinItemProps & WinChangeProps<WinValue> & { SelectedIndex?: number }
): React.JSX.Element {
	const items = itemsOf(props)
	const [selected, setSelected] = useControllable(
		props.SelectedIndex ?? props.value,
	...
	return (
		<div className={cx("win-items-view", props.class)}>
			{items.map((item, index) => (
				<button
					key={index}
					type="button"
					className={cx("win-item", selected === index ? "selected" : undefined)}
					onClick={() => {
						setSelected(index)
						callback<WinItem>(props, "onItemClick", "ItemClick")?.(item)
					}}
				>
					{props.renderItem ? props.renderItem(item, index) : itemLabel(item)}
				</button>
			))}
		</div>
	)
...
export function WinListBox(
	...
	return <WinItemsView {...props} className={cx("win-list-box", props.className)} />
}
export function WinListView(
	props: WinItemProps & WinChangeProps<WinValue> & { SelectedIndex?: number }
): React.JSX.Element {
	return <WinItemsView {...props} className={cx("win-list-view", props.className)} />
}
export function WinGridView(
	props: WinItemProps & WinChangeProps<WinValue> & { SelectedIndex?: number }
): React.JSX.Element {
	return <WinItemsView {...props} className={cx("win-grid-view", props.className)} />
}
export function WinTreeView(
	props: WinItemProps & WinChangeProps<WinValue> & { RootItems?: WinItem[] }
): React.JSX.Element {
	const renderTree = (items: WinItem[], level = 0): ReactNode =>
		items.map((item, index) => {
			const record =
				typeof item === "object" && item !== null ? (item as Record<string, unknown>) : {}
			const children = Array.isArray(record.children) ? (record.children as WinItem[]) : []
			return (
				<div
					key={String(level) + "-" + String(index)}
					className="win-tree-node"
					style={{ paddingLeft: level * 16 }}
				>
					<button
						type="button"
						onClick={() => callback<WinItem>(props, "onItemClick", "ItemClick")?.(item)}
					>
						{itemLabel(item)}
					</button>
	...
	return (
		<div className={cx("win-tree-view", props.class)}>
			{renderTree(props.RootItems ?? itemsOf(props))}
		</div>
	)
}
export function WinComboBox(
	...
	const [selected, setSelected] = useControllable(
		props.SelectedIndex ?? props.value,
		initial >= 0 ? initial : -1,
		callback<WinValue>(props, "onValueChange", "onUpdate:SelectedIndex", "onUpdate:modelValue")
	)
	const [open, setOpen] = useState(false)
	const selectedItem =
		typeof selected === "number" ? items[selected] : items.find((item) => item === selected)
	return (
		<div className={cx("win-combo-box", open ? "open" : undefined, props.class)}>
			<button
				type="button"
				className="win-combo-button"
				disabled={props.IsEnabled === false}
				onClick={() => setOpen(!open)}
			>
				{selectedItem
					? itemLabel(selectedItem)
					: (props.PlaceholderText ?? props.Header ?? "")}
				<span className="chevron-animate" />
			</button>
			{open && (
				<div className="win-combo-flyout">
					{items.map((item, index) => (
						<button
							key={index}
							type="button"
							className={cx(
								"win-combo-item",
								selected === index ? "selected" : undefined
							)}
							onClick={() => {
								setSelected(index)
								setOpen(false)
...
export function WinAutoSuggestBox(
	props: WinProps & WinChangeProps<string> & { Value?: string; PlaceholderText?: string }
): React.JSX.Element {
	return (
		<WinTextBox
...
export function WinCalendarView(
	props: WinProps &
		WinChangeProps<Date> & { Date?: Date; SelectedDate?: Date; MinDate?: Date; MaxDate?: Date }
): React.JSX.Element {
	const [date, setDate] = useControllable(
		props.SelectedDate ?? props.value,
		props.Date ?? new Date(),
		callback<Date>(props, "onValueChange", "onUpdate:SelectedDate")
	)
	return (
		<input
			type="date"
			className={cx("win-calendar-view", props.class)}
			value={date instanceof Date ? date.toISOString().slice(0, 10) : ""}
			min={props.MinDate?.toISOString().slice(0, 10)}
			max={props.MaxDate?.toISOString().slice(0, 10)}
			onChange={(event) => setDate(new Date(event.currentTarget.value + "T00:00:00"))}
		/>
	)
}
export function WinCalendarDatePicker(
	props: WinProps & WinChangeProps<Date> & { Date?: Date; SelectedDate?: Date }
): React.JSX.Element {
	return (
		<label className={cx("win-calendar-date-picker", props.class)}>
			{props.Header && <WinTextBlock Text={props.Header} />}
			<WinCalendarView {...props} />
		</label>
	)
}
export function WinDatePicker(
	props: WinProps & WinChangeProps<Date> & { Date?: Date; SelectedDate?: Date }
): React.JSX.Element {
	return <WinCalendarDatePicker {...props} className={cx("win-date-picker", props.className)} />
}
export function WinTimePicker(
	...
	const [time, setTime] = useControllable(
		...
		String(props.defaultValue ?? ""),
		callback<string>(props, "onValueChange", "onUpdate:Time")
	)
	return (
		<label className={cx("win-time-picker", props.class)}>
			{props.Header && <WinTextBlock Text={props.Header} />}
			<input
				type="time"
				value={String(time)}
...
export function WinColorPicker(
	props: WinProps & WinChangeProps<string> & { Color?: string }
): React.JSX.Element {
	const [color, setColor] = useControllable(
		props.Color ?? props.value,
		String(props.defaultValue ?? "#0067c0"),
		callback<string>(props, "onValueChange", "onUpdate:Color")
	)
	return (
		<label className={cx("win-color-picker", props.class)}>
			{props.Header && <WinTextBlock Text={props.Header} />}
			<input
				type="color"
				value={String(color)}
...
export function WinPickerColumn(
	props: WinItemProps & WinChangeProps<WinValue> & { SelectedIndex?: number }
): React.JSX.Element {
	return <WinComboBox {...props} className={cx("win-picker-column", props.className)} />
}
export function WinPipsPager(
	props: WinProps &
		WinChangeProps<number> & { NumberOfPages?: number; SelectedPageIndex?: number }
): React.JSX.Element {
	const [selected, setSelected] = useControllable(
		props.SelectedPageIndex ?? props.value,
		0,
		callback<number>(props, "onValueChange", "onUpdate:SelectedPageIndex")
	)
	return (
		<div className={cx("win-pips-pager", props.class)}>
			{Array.from({ length: props.NumberOfPages ?? 1 }, (_, index) => (
				<button
					key={index}
					type="button"
...
export function WinFlipView(
	props: WinItemProps & WinChangeProps<number> & { SelectedIndex?: number }
): React.JSX.Element {
	const items = itemsOf(props)
	const [selected, setSelected] = useControllable(
		props.SelectedIndex ?? props.value,
	...
	return (
		<div className={cx("win-flip-view", props.class)}>
			{items[selected] &&
				(props.renderItem
					? props.renderItem(items[selected], selected)
					: itemLabel(items[selected]))}
			<button
				type="button"
				onClick={() => setSelected(Math.min(items.length - 1, Number(selected) + 1))}
			>
				›
			</button>
		</div>
...
export function WinPivot(
	...
	return (
		<div className={cx("win-pivot", props.class)}>
			<div className="win-pivot-tabs">
				{items.map((item, index) => (
					<button
						key={index}
						type="button"
			...
			<div className="win-pivot-content">
				{props.children ?? (items[selected] && itemLabel(items[selected]))}
			</div>
		</div>
	)
}
...
export function WinSelectorBar(
	...
	return <WinPivot {...props} className={cx("win-selector-bar", props.className)} />
}
...
export function WinSemanticZoom(props: WinProps): React.JSX.Element {
	return <div className={cx("win-semantic-zoom", props.class)}>{props.children}</div>
}
export function WinNavigationView(
	props: WinItemProps &
		WinChangeProps<WinValue> & { MenuItems?: WinItem[]; PaneDisplayMode?: string }
): React.JSX.Element {
	const items = props.MenuItems ?? itemsOf(props)
	const [selected, setSelected] = useControllable(
		props.value,
		0,
		callback<WinValue>(props, "onValueChange", "onSelectionChanged")
	)
	return (
		<div className={cx("win-navigation-view", props.class)}>
			<aside className="win-navigation-pane">
				<div className="win-navigation-header">{props.Header ?? props.PaneTitle}</div>
				{items.map((item, index) => (
					<button
						key={index}
						type="button"
			...
			<main className="win-navigation-content">{props.children}</main>
		</div>
	)
}
export function WinSplitView(props: WinProps & { IsPaneOpen?: boolean }): React.JSX.Element {
	return (
		<div
			className={cx(
				"win-split-view",
				props.IsPaneOpen ? "pane-open" : undefined,
				props.class
			)}
		>
			<aside className="win-split-pane">{props.Pane}</aside>
			<main>{props.children}</main>
		</div>
	)
}
export function WinBreadcrumbBar(props: WinItemProps): React.JSX.Element {
	return (
		<nav className={cx("win-breadcrumb-bar", props.class)}>
			{itemsOf(props).map((item, index) => (
				<span key={index}>
...
export function WinAppBarButton(props: WinProps): React.JSX.Element {
	return <WinButton {...props} className={cx("win-app-bar-button", props.className)} />
}
export function WinAppBarToggleButton(
	props: WinProps & WinChangeProps<boolean>
): React.JSX.Element {
	return (
		<WinToggleButton {...props} className={cx("win-app-bar-toggle-button", props.className)} />
	)
}
export function WinAppBarSeparator(props: WinProps): React.JSX.Element {
	return <span className={cx("win-app-bar-separator", props.class)} aria-hidden="true" />
}
export function WinCommandBar(props: WinItemProps): React.JSX.Element {
	return (
		<div className={cx("win-command-bar", props.class)}>
			{props.children ?? renderItems(props)}
		</div>
	)
}
export function WinCommandBarFlyout(props: WinProps): React.JSX.Element {
	return <WinFlyout {...props} className={cx("win-command-bar-flyout", props.className)} />
}
export function WinDropDownButton(props: WinProps & { Items?: WinMenuItem[] }): React.JSX.Element {
	return <WinSplitButton {...props} className={cx("win-dropdown-button", props.className)} />
}
export function WinSplitButton(props: WinProps & { Items?: WinMenuItem[] }): React.JSX.Element {
	return (
		<div className={cx("win-split-button", props.class)}>
			<WinButton {...props} className="win-split-main-button" />
			{props.Items && (
				<WinMenuFlyout
					Items={props.Items}
					Trigger={<WinButton aria-label="Open menu">⌄</WinButton>}
				/>
			)}
		</div>
	)
...
export function WinMenuBar(props: WinItemProps): React.JSX.Element {
	return (
		<nav className={cx("win-menu-bar", props.class)}>
			{props.children ?? renderItems(props)}
		</nav>
	)
}
...
export function WinCaptureElement(props: WinProps): React.JSX.Element {
	return <WinScrollViewer {...props} className={cx("win-capture-element", props.className)} />
}
export function WinMediaPlayerElement(
	props: WinProps & {
		Source?: string
		AutoPlay?: boolean
		IsLooping?: boolean
	...
	return (
		<video
			className={cx("win-media-player", props.class)}
			src={props.Source}
			autoPlay={props.AutoPlay}
			loop={props.IsLooping}
			muted={props.IsMuted}
			controls={props.AreTransportControlsEnabled !== false}
		>
			{props.children}
		</video>
	)
}
export function WinAnimatedVisualPlayer(props: WinProps): React.JSX.Element {
	return (
		<div className={cx("win-animated-visual-player", props.class)}>
			{props.children ?? props.Content}
		</div>
	)
}
export function WinParallaxView(props: WinProps): React.JSX.Element {
	return <WinScrollViewer {...props} className={cx("win-parallax-view", props.className)} />
}
export function WinPullToRefresh(props: WinProps): React.JSX.Element {
	return (
		<div
			className={cx("win-pull-to-refresh", props.class)}
			onPointerDown={() =>
				callback<unknown>(props, "onRefreshRequested", "RefreshRequested")?.(undefined)
			}
		>
			{props.children}
		</div>
...
export function WinRefreshVisualizer(props: WinProps): React.JSX.Element {
	return (
		<span className={cx("win-refresh-visualizer", props.class)}>{props.children ?? "↻"}</span>
	)
}
export function WinRepeatButton(props: WinProps): React.JSX.Element {
	return <WinButton {...props} className={cx("win-repeat-button", props.className)} />
}
export function WinSwipeControl(props: WinProps): React.JSX.Element {
	return <div className={cx("win-swipe-control", props.class)}>{props.children}</div>
}
...
export function WinTitleBar(props: WinProps): React.JSX.Element {
	return (
		<header className={cx("win-title-bar", props.class)}>
			{props.children ?? <WinTextBlock Text={props.Title ?? props.Content} />}
		</header>
	)
}
